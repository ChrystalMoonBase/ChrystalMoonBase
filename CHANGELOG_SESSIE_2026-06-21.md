...
# Denkpunten-sessie aanpassingen (23 juni 2026)

Na een sessie van 10 kritische denkpunten zijn deze concrete repo-correcties gedaan:

## Communicatie naar aarde (data-relay-to-earth.md) — belangrijke correctie
- "From the near-side Peary rim, Earth is always visible" was te stellig. Gecorrigeerd:
  Peary ligt op de limb, Earth staat laag op de horizon, zichtbaar een groot deel van de
  tijd maar NIET gegarandeerd 100% (libratie + terrein kunnen line-of-sight onderbreken).
  Percentage moet met LOLA-topografie bevestigd worden (vgl. zuidpool ~50–93%).
- Twee onafhankelijke paden toegevoegd: directe pieklink (met data-buffering) + ESA
  relais (Lunar Pathfinder 2026 / Moonlight ~2030, een dienst — caveat: nu zuidpool-
  prioriteit, noordpool-dekking met ESA te bevestigen).
- Signaalvertraging gecorrigeerd: was "1.3–2.6 s one-way / 2.6–5.2 round-trip" (fout) →
  nu ~1.3 s one-way, ~2.6 s round-trip (consistent met rest van repo).

## Mast + bloom als twee harde mission-enders (mast-overview.md)
- Bloom-deployment toegevoegd als even-kritiek (mogelijk kritieker) faalpunt naast de
  mast: pod opent niet → robots opgesloten, geen herstelpad.
- 25-batterij-buffer per robot expliciet genoemd: tijdelijke mast-onderbreking
  overbrugbaar op eigen accu; alleen deployment-falen is onherstelbaar.
- Uitkomst van totale mislukking bewust open gelaten (hangt af van data die nog niet
  bestaat) i.p.v. een fictief reddingsscenario.

## Vertraging-consistentie (laser-communication.md)
- "2.6 to 5.2 seconds" round-trip (fout) → ~2.6 s round-trip, consistent gemaakt.

## Geverifieerd, geen wijziging nodig
- Stof-mitigatie via "sinteren voor de voet uit" stond al correct in leg-architecture.md
  (regel 128–132). Berechja's intuïtie klopte met bestaande tekst.

Volledige denkpunten-analyse: OPEN_DENKPUNTEN.md (werkdocument, 10 punten + sub-punten).

## Batterij-nodes uitleg (CMB8LF_v1 BOM v6 + vacuum)
- Verklarend kader toegevoegd bij "Segment Batterijen (25 nodes)": waar het getal 25
  vandaan komt (één node per bewegend segment, niet per poot), waarom (fault-tolerance:
  valt één node uit, werkt de rest), en dat 25 een schaalbare werkwaarde is — principe
  vast, exact aantal door engineering bepaald. Toegevoegd op verzoek: een vraag die bij
  review opkomt, zullen anderen ook stellen.

## Twee nieuwe robotklassen — concept (23 juni 2026)
Naar aanleiding van de "wielen vs poten"-discussie (oom's vraag) en de erkenning dat
transport en graven aparte taken zijn met aparte lichamen:
- **systems/robot-bobcat/README.md** — graver/dozer ("gerobotiseerde Bobcat"). Rol
  concreet (graven + egaliseren, werkt vóór de octopods uit, voedt ISRU), exacte specs
  expliciet voor professionals. Eigen klasse om fysieke reden (kracht vs stilstand), geen
  vierde specialist-octopod.
- **systems/robot-transport/README.md** — hauler op wielen over gesinterde paden.
  Kernidee: **standaard canister** (containerprincipe — elke machine koppelt dezelfde
  bak). Octopod-fallback expliciet (geen harde single-point dependency).

Principe vastgelegd: "geen specialisten" geldt BINNEN de octopod-bouwvloot; aparte klassen
(Bobcat, transport, cable-worm B1) zijn andere lichamen voor andere fysieke taken, elk met
redundantie-door-aantal. Exacte maten/aantallen wachten op prototype-metingen.

NB: alles valt onder "robots/drones van CMB"; definitieve plate-style naamgeving (type
direct afleesbaar uit de code) volgt later bij een complete hernoeming van de hele familie.
