...
### Tool and Task Operation
Every robot has the full capability set integrated — sensors, sintering laser, and infrared annealing lamps are always on board. The onboard AI runs the relevant operational module for whatever **task** the swarm AI has currently assigned, not for a swapped-in head:
- **Sintering task** — controls laser scanning patterns and power levels.
- **Annealing/cooling task** — manages the integrated infrared lamp power based on thermal-camera feedback (this travels with every sintering robot; broader-area thermal management is assigned as needed).
- **Observation (Watcher) task** — manages the full integrated sensor suite and data prioritisation.
- **Heavy-tool task** — when the bulldozer blade or digger is fitted via the EPM arm interface, the corresponding module controls blade depth / compaction or drum excavation. These two heavy tools are the only physically swapped items (see `tools/README.md`).

All of these modules run on the same robot; which one is active is set by task assignment in software, not by changing hardware.
