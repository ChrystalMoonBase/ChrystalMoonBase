...
**Minimum operational leg count:** 6 legs (estimated). Below 6 legs, stable locomotion becomes difficult to guarantee on uneven terrain, and the robot would likely be assigned stationary tasks only.
...
5. **Minimum leg count** — 6 legs is an estimate. Actual minimum for stable operation on Peary rim terrain requires locomotion simulation with realistic terrain models
