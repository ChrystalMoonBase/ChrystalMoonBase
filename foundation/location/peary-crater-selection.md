...
4. **Earth visibility** — line-of-sight for communications a high fraction of the time (relay-backed for any gaps)
...
From the near-side rim peaks of Peary crater, Earth is visible low on the horizon for a large fraction of the time — but not guaranteed 100%. Because Earth sits low over the polar horizon, lunar libration and local terrain can interrupt the direct line of sight at times; the exact percentage must be confirmed against high-resolution LOLA topography for the specific peak. This is still a major communications advantage over far-side locations (which never see Earth directly), and the gaps can be covered two ways: data buffering for the direct link, and a lunar relay satellite (ESA's Lunar Pathfinder / Moonlight). See `systems/ai-and-monitoring/data-relay-to-earth.md` for the full treatment.
